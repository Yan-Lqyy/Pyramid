...
# --- Pattern Interfaces and Triangle Pattern Implementation ---
...
class TrianglePattern(IPattern):
    """
    Implements a right-triangle board of ORDER rows.
    Only positions with x <= y are valid.
    We maintain:
      - original_matrix: the fixed mapping of valid cells (numbered 1..N)
      - occupancy: a working copy that is modified as pieces are placed.
    """
    ORDER = 10  # Number of rows (so there are ORDER*(ORDER+1)/2 valid cells)

    def __init__(self):
        self.matrix: List[int] = []  # Mapping of full board positions to valid cell numbers.
        index = 1
        for y in range(self.ORDER):
            for x in range(self.ORDER):
                if x <= y:
                    self.matrix.append(index)
                    index += 1
                else:
                    self.matrix.append(0)  # Outside the triangle.
        ...
        for y in range(self.ORDER):
            for x in range(self.ORDER):
                ...
                for p in piece.get_points():
                    px, py = p.x + x, p.y + y
                    if not (0 <= px < self.ORDER and 0 <= py < self.ORDER and px <= py and self.occupancy[py * self.ORDER + px] != 0):
                        valid = False
                        break
                if valid:
                    step = Step(piece.block_index, piece.shape_index, x, y)
                    for p in piece.get_points():
                        # Use the unmodified mapping from original_matrix.
                        step.indices.append(self.original_matrix[(p.y + y) * self.ORDER + (p.x + x)])
                    steps.append(step)
        return steps
    ...
    def format_matrix(self, solution: List[Step]) -> List[List[int]]:
        ...
        For each valid cell (numbered from 1 to N), if a piece was placed there,
        ...
        for y in range(self.ORDER):
            ...
            for x in range(self.ORDER):
                if x <= y:
                    row.append(board_display[cell_num])
                    cell_num += 1
            result.append(row)
        return result
    ...
    def is_valid_placement(self, piece: Piece, x: int, y: int, current_matrix: List[int]) -> bool:
        ...
        for p in piece.get_points():
            px, py = p.x + x, p.y + y
            # Check board boundaries and triangle shape (x <= y) and occupancy
            if not (0 <= px < self.ORDER and 0 <= py < self.ORDER and px <= py and current_matrix[py * self.ORDER + px] != 0):
                return False  # Placement is invalid
        return True  # All points are within valid and unoccupied cells
    ...
    def is_board_full(self) -> bool:
        """Checks if all valid cells in the triangle pattern have been filled."""
        # A cell is considered filled if its value in the occupancy matrix is 0.
        return all(cell == 0 for cell in self.occupancy if cell != 0)  # Only check valid cells


# --- Output Functions ---

def visualize_board_colored(pattern: TrianglePattern, solution: List[Step] = None):
    """Visualize the triangle board using Rich markup."""
    board_matrix = pattern.format_matrix(solution if solution else [])
    cell_width = 2
    print("\n[bold]Current Board State:[/bold]")
    for y_index, row in enumerate(board_matrix):
        row_segments = []
        for piece_index in row:
            if piece_index == -1:
                cell_content = "."
                padded_content = f"{cell_content:<{cell_width}}"
                rich_text_cell = Text(padded_content)
            else:
                piece_name = PIECE_MAP[piece_index]
                color = RICH_COLORS[piece_index % len(RICH_COLORS)]
                padded_content = f"{piece_name:<{cell_width}}"
                rich_text_cell = Text(padded_content, style=color)
            row_segments.append(rich_text_cell)
        console.print(f"[dim]Row {y_index}:[/dim] ", end="")
        console.print(*row_segments, sep="")

def display_board(pattern: TrianglePattern, pre_placed_solution: List[Step] = None):
...
def main():
    pattern = TrianglePattern()  # Create the board.
    pre_placed_solution: List[Step] = []  # List to store placements.
    used_piece_indices = set()  # Avoid placing the same piece twice.

    print("[bold blue]Triangle Pattern Puzzle Solver - Placement Debug[/bold blue]")
    ...
    while True:
        ...
        # Normalize to uppercase so that input is case-insensitive.
        ...
        # For shape selection, use just numbers as strings.
        ...
        # Convert the selection to an index (zero-based).
        ...
        while True:
            ...
            if not coords:
                break  # Allow cancelling coordinate input to go back to shape selection
            try:
                ...
                if pattern.place_piece(selected_piece_shape, x_coord, y_coord):
                    ...
                    # Use the unmodified mapping to record the valid cell numbers.
                    for p in selected_piece_shape.get_points():
                        idx = pattern.original_matrix[(p.y + y_coord) * pattern.ORDER + (p.x + x_coord)]
